    ...
    <aside className="space-y-5">
      ...
      <div className="bg-white rounded-xl p-5 shadow-sm border border-gray-50">
        <div className="text-center">
          ...
          <h3 className="text-base font-bold text-gray-900 mb-2">Funnel Effect.</h3>
          ...
          <div className="flex justify-center space-x-2">
            <a href="#" className="w-7 h-7 bg-gray-100 hover:bg-red-500 hover:text-white rounded-full flex items-center justify-center text-gray-600 transition-colors">
      ...
      {/* Popular Posts */}
      <div className="bg-white rounded-xl p-5 shadow-sm border border-gray-50">
        <h3 className="text-base font-bold text-gray-900 mb-4 pb-2 border-b border-gray-100">Popular Posts</h3>
            ...
            <div key={index} className="group">
              <div className="flex space-x-3">
                ...
                <div className="flex-1 min-w-0">
                  <h4 className="text-xs font-medium text-gray-900 group-hover:text-red-500 transition-colors line-clamp-2 mb-1">
                    <Link href={`/posts/${post.title.toLowerCase().replace(/\s+/g, '-')}`}>
                      {post.title}
                    </Link>
                  </h4>
                  <div className="flex items-center text-xs text-gray-500">
                    <span className="bg-red-500 text-white px-2 py-1 rounded-full text-xs">{post.category}</span>
      ...
      {/* Categories */}
      <div className="bg-white rounded-xl p-5 shadow-sm border border-gray-50">
        <h3 className="text-base font-bold text-gray-900 mb-4 pb-2 border-b border-gray-100">Explore Topics</h3>
        <div className="space-y-2">
          {categories.map((category, index) => (
            <div key={index}>
              <Link 
                href={`/category/${category.name.toLowerCase()}`}
                className="flex items-center justify-between py-1.5 text-gray-700 hover:text-red-500 transition-colors group"
              >
                <span className="text-xs font-medium">{category.name}</span>
                <span className="text-xs text-gray-500 bg-gray-100 px-2 py-1 rounded-full group-hover:bg-red-50 group-hover:text-red-500 transition-colors">
                  {category.count}
                </span>
              </Link>
            </div>
      ...
      <div className="bg-red-500 rounded-xl p-5 text-white">
        ...
        <form className="space-y-2">
          <input 
            ...
            className="w-full px-3 py-2.5 rounded-lg text-gray-900 placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-white/50 text-sm"
          />
          <button 
            type="submit"
